...
import AssignBookletModal from "./AssignBookletModal";
import {
  X,
  FileText,
  Calendar,
...
const [assignOpen, setAssignOpen] =
  ...
  function handleAddBooklet(item: any) {
...
async function handleGenerateRAT() {
          ...
          <div className="space-y-6 p-6">
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                                ...
                                {(assignments[item.id] ?? []).length === 0 ? (
                                    ...
                                    <div className="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700 inline-block">

                                        Assigned ({(assignments[item.id] ?? []).length}/{item.quantity})

                                    </div>

                                    {(assignments[item.id] ?? []).map((booklet: any) => (
            ...
            onSelect={(booklet) => {
                ...
                setAssignments((prev) => {
                    ...
                    // Prevent assigning the same booklet twice
                    if (
                        current.some(
                            (b) => b.id === booklet.id
                        )
